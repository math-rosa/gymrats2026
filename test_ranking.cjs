...
const rawData = parseCSV(str);
const { data, headers } = csvToJson(rawData);
...
const restData = sortedList.filter(item => item.rank > 3);
console.log("Total groups:", sortedList.length);
console.log("Groups in rest:", restData.length);
console.log("Last group:", sortedList[sortedList.length - 1].name, "rank:", sortedList[sortedList.length - 1].rank, "total:", sortedList[sortedList.length - 1].total);
